...
- (void)showErrorMessage:(NSError *)error {
    if (error.code == KOErrorCancelled) {
        ...
        [UIAlertView showMessage:[NSString stringWithFormat:@"에러! code=%d, msg=%@", (int) error.code, description != nil ? description : @"unknown error"]];
    }
}
            ...
            case 1: {
                [KOSessionTask profileUpdateTaskWithProperties:[self userFormData] completionHandler:^(BOOL success, NSError *error) {
                    if (error) {
                        ...
                        [UIAlertView showMessage:@"프로필 업데이트에 성공하셨습니다."];
                    }
                }];
            }
                break;
...
- (void)profileImageTapped:(UITapGestureRecognizer *)recognizer {
    if (!_user || !_user.properties[@"profile_image"]) {
        return;
    }
    
    NSString *profileImage = _user.properties[@"profile_image"];
