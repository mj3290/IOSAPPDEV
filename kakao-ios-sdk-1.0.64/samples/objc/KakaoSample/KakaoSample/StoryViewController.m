...
- (void)postNote {
    [KOSessionTask storyPostNoteTaskWithContent:@"post note test"
                              ...
                              completionHandler:^(KOStoryPostInfo *post, NSError *error) {
                                  if (error) {
                                      [UIAlertView showMessage:[NSString stringWithFormat:@"post note error = %@", error.description]];
                                  } else {
                                      _lastStoryID = post.ID;
                                      [UIAlertView showMessage:[NSString stringWithFormat:@"post note success = %@", _lastStoryID]];
                                  }
                              }];
}
...
- (void)storyPostImages:(NSArray *)imageURLs {
    [KOSessionTask storyPostPhotoTaskWithImageUrls:imageURLs
                                 ...
                                 completionHandler:^(KOStoryPostInfo *post, NSError *error) {
                                     if (error) {
                                         [UIAlertView showMessage:[NSString stringWithFormat:@"post photo error = %@", error.description]];
                                     } else {
                                         _lastStoryID = post.ID;
                                         [UIAlertView showMessage:[NSString stringWithFormat:@"post photo success = %@", _lastStoryID]];
                                     }
                                 }];
}
...
- (void)postLink {
    [KOSessionTask storyGetLinkInfoTaskWithUrl:@"https://developers.kakao.com" completionHandler:^(KOStoryLinkInfo *link, NSError *error) {
        if (error) {
            [UIAlertView showMessage:[NSString stringWithFormat:@"get link info error = %@", error.description]];
        } else {
...
- (void)storyPostLink:(KOStoryLinkInfo *)linkInfo {
    [KOSessionTask storyPostLinkTaskWithLinkInfo:linkInfo
                               ...
                               completionHandler:^(KOStoryPostInfo *post, NSError *error) {
                                   if (error) {
                                       [UIAlertView showMessage:[NSString stringWithFormat:@"post link error = %@", error.description]];
                                   } else {
                                       _lastStoryID = post.ID;
                                       [UIAlertView showMessage:[NSString stringWithFormat:@"post link success = %@", _lastStoryID]];
                                   }
                               }];
}
...
- (void)getMyStory {
    ...
    [KOSessionTask storyGetMyStoryTaskWithMyStoryId:_lastStoryID completionHandler:^(KOStoryMyStoryInfo *myStory, NSError *error) {
        if (error) {
            [UIAlertView showMessage:[NSString stringWithFormat:@"my story error = %@", error.description]];
        } else {
            [UIAlertView showMessage:[NSString stringWithFormat:@"my story success = %@", myStory.description]];
        }
    }];
}
...
- (void)getMyStories {
    ...
    [KOSessionTask storyGetMyStoriesTaskWithLastMyStoryId:_lastStoryID completionHandler:^(NSArray *myStories, NSError *error) {
        if (error) {
            [UIAlertView showMessage:[NSString stringWithFormat:@"my stories error = %@", error.description]];
        } else {
            [UIAlertView showMessage:[NSString stringWithFormat:@"my stories success = %@", myStories.description]];
        }
    }];
}
...
- (void)deleteLastStory {
    ...
    [KOSessionTask storyDeleteMyStoryTaskWithMyStoryId:_lastStoryID completionHandler:^(NSError *error) {
        if (error) {
            [UIAlertView showMessage:[NSString stringWithFormat:@"delete error = %@", error.description]];
        } else {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    [KOSessionTask storyMultiImagesUploadTaskWithImages:@[image] completionHandler:^(NSArray *imageUrls, NSError *error) {
        if (error) {
            [UIAlertView showMessage:[NSString stringWithFormat:@"upload photo error = %@", error.description]];
        } else {
