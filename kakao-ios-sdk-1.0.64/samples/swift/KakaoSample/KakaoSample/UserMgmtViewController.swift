...
class UserMgmtViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func profileImageTapped(_ recognizer: UITapGestureRecognizer) {
        if user == nil || user?.properties["profile_image"] == nil {
            return
        }
        
        if let imageUrl = user?.properties["profile_image"] as? String , imageUrl.isEmpty {
            return
        }
        
        self.performSegue(withIdentifier: "ProfileImage", sender: user?.properties["profile_image"])
    }
